...
class MNIST:
  def __init__(self, batch_size=512):
    ...
    self.train_loader = DataLoader(self.train_data, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=num_workers, pin_memory=True)
    self.test_data = datasets.MNIST('.', train=False, transform=totensor, target_transform=None, download=True)
    self.test_loader = DataLoader(self.test_data, batch_size=batch_size, shuffle=False, num_workers=num_workers, pin_memory=True)
...
class BagOfWords:
  def __init__(self, n, batch_size):
    ...
    self.train_loader = DataLoader(BoWDataset(self.train_data), batch_size=batch_size, shuffle=True, drop_last=True, pin_memory=True, num_workers=num_workers)
    self.test_loader = DataLoader(BoWDataset(self.test_data), batch_size=batch_size, shuffle=False, drop_last=False, pin_memory=True, num_workers=num_workers)
    self.train_loader.reset = lambda: None
    self.train_data.reset = lambda: None
